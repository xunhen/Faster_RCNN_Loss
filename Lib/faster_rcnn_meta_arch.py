...
class FasterRCNNMetaArch(model.DetectionModel):
    ...
    def predict(self, preprocessed_inputs, true_image_shapes):
        ...
        if not self._is_training and self._rpn_type == 'without_rpn':
            if self._replace_rpn_arg and self._replace_rpn_arg.get('type', 'gt') == 'gt':
                proposal_bboxes, proposal_scores, vaildation = self._get_gt_data(
                    scale=self._replace_rpn_arg('scale', 1.0))
                proposal_scores = tf.squeeze(proposal_scores, axis=-1)
            else:
                proposal_bboxes, proposal_scores, vaildation = self._get_rpn_box()
                proposal_scores = tf.squeeze(proposal_scores, axis=-1)
    ...
    def _scale_bbox(self, boxes, scale=1.0):
        ...
        ymin = tf.maximum(0, ymin + height * (1 - scale) / 2)
        xmin = tf.maximum(0, xmin + width * (1 - scale) / 2)
        ymax = tf.maximum(0, ymax + height * (scale - 1) / 2)
        xmax = tf.maximum(0, xmax + width * (scale - 1) / 2)
    ...
    def _per_batch_padding(self, args):
        ...
        Args:
            ...
            args[31: max_number. A int tensor with shape [,] representing
    ...
    def _get_gt_data(self, scale=None, agnostic=True):
        ...
        groundtruth_boxs, vaildations = \
            shape_utils.static_or_dynamic_map_fn(
                self._per_batch_padding, [groundtruth_boxs, tf.fill(
                    [len(groundtruth_boxs), ], self.max_num_proposals)])
    ...
    def provide_rpn_box(self, rpn_boxes_list):
        self._rpn_boxes_list = rpn_boxes_list

    def _get_rpn_box(self):
        rpn_boxes, vaildations = \
            shape_utils.static_or_dynamic_map_fn(
                self._per_batch_padding, [self._rpn_boxes_list, tf.fill(
                    [len(self._rpn_boxes_list), ], self.max_num_proposals)])
    ...
    def provide_filter_box(self, filter_boxes_list):
        """
